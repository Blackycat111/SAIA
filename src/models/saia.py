...
class SAIA(GeneralRecommender):
    def __init__(self, config, dataset):
        super(SAIA, self).__init__(config, dataset)
        ...
        self.inter_tensor = self.scipy_sparse_to_torch_tensor(self.interaction_matrix).to(self.device)
    ...
    def scipy_sparse_to_torch_tensor(self, mat):
    ...
    def GrowthScore(self, old_embedding, new_embedding):
        if self.distance == "XiangDuiOuSi":
            ...
            if self.old_new_dir == "new":
                ...
                raise ValueError("Error: Invalid old_new_dir")
            d_all = d_old + d_new
            score_old = d_old / d_all
            score_new = d_new / d_all
        else:
            raise ValueError("Error: Unsupported distance")
    ...
    def post_epoch_processing(self, epoch):
        # save model to log path
        torch.save(self.state_dict(), os.path.join(self.log_path, f"{epoch}.pt"))

        users_embeddings, items_embeddings = torch.split(self.all_embeds, [self.n_users, self.n_items], dim=0)

        selected_items = users_embeddings[101:3101]
        x1_values = selected_items[:, 5].detach().cpu()
        y1_values = selected_items[:, 6].detach().cpu()
        z1_values = selected_items[:, 7].detach().cpu()
        to_pth = torch.stack([x1_values, y1_values, z1_values], dim=0)
        torch.save(to_pth, os.path.join(self.log_path, f"{epoch}_features.pt"))

        return f'save to {self.log_path}_{epoch}.pt'
    ...
    def forward(self, adj, train=False):
        ...
        self.content_embeds = all_embeddings
        ...
        image_prefer = self.gate_image_prefer(self.content_embeds)
        text_prefer = self.gate_text_prefer(self.content_embeds)
        ...
        self.all_embeds = self.content_embeds + side_embeds

        all_embeddings_users, all_embeddings_items = torch.split(self.all_embeds, [self.n_users, self.n_items], dim=0)

        if train:
            return all_embeddings_users, all_embeddings_items, side_embeds, self.content_embeds
...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, x):
        ...
        pe = self.pe[:seq_len]
        noise_std = 0.3
        noise = torch.randn_like(pe) * noise_std

        pe_modified = pe + noise
        return x + pe_modified
